...
class ProcessApproval extends React.Component {
    ...
    render() {
        ...
        return <section className="ProcessApproval">
            <ProcessApprovalTab id={th.props.location.query["e"]} current="ProcessApprover"
                                allSearchArg={th.state.allSearchArg}/>
            <h3 className="boxGroupTit"><p><span>审批流程</span><span className="work-flow-title">{workFlowTitle}</span></p>
            </h3>
            <article>
                <div className="mgT10">
                    <ApprovalControlNode2 guid={th.state.guid} allSearchArg={th.state.allSearchArg}
                                          callback={th.evSetRemarkTxt.bind(th)}/>
                </div>
            </article>
        </section>
