...
import "../css/processApproval.less";
...
class ProcessApproval extends React.Component {
    ...
    componentDidMount() {
        const {e, dataKey} = this.props.location.query;
        CommonService.getWorkFlowTitle(e, dataKey)
            .then(res => res.rows)
            .then(workFlowTitle => {
                this.setState({
                    workFlowTitle
    ...
    render() {
        ...
        return <section className="ProcessApproval">
            ...
            <h3 className="boxGroupTit"><p>
                <span>审批流程</span>
                <span className="work-flow-title">{workFlowTitle}</span>
            </p></h3>
            <article>
                <table className="table PATextarea" width="100%">
                    ...
                    <tr>
                        <th>备注</th>
                        <td>
                            <textarea value={this.state.remarkTxt} onChange={this.EVENT_CHANGE_TEXTAREA.bind(this)}
                ...
                <div className="mgT10">
                    <ApprovalControlNode AppState={this.AppState} guid={this.state.guid} data={this.state.remarkTxt}
                                         allSearchArg={this.state.allSearchArg}/>
                </div>
